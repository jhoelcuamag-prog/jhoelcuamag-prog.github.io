...
$products = $pdo->query("SELECT id, product_name, product_code, unit_price FROM products ORDER BY product_name")->fetchAll();
$error = '';
$success = '';

// Generate transaction ID
$transaction_id = 'INC-' . date('Ymd') . '-' . strtoupper(uniqid());
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $pdo->prepare("INSERT INTO financial_transactions (transaction_id, transaction_type, date, description, item_name, quantity, unit_price, total_amount, product_id, created_by) VALUES (?, 'income', ?, ?, ?, ?, ?, ?, ?, ?)");
    
    if($stmt->execute([$transaction_id, $date, $description, $item_name, $quantity, $unit_price, $total_amount, $product_id, $_SESSION['user_id']])) {
        $success = "Income transaction saved successfully!";
        $transaction_id = 'INC-' . date('Ymd') . '-' . strtoupper(uniqid());
    } else {
...
<head>
    ...
    <title>Add Income - Lombriks Admin</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-save { background: linear-gradient(90deg, #27ae60, #2ecc71); color: white; border: none; border-radius: 20px; padding: 10px 30px; }
        .btn-save:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(39,174,96,0.4); }
...
<body>
    ...
    <div class="sidebar">
        ...
        <nav class="nav flex-column">
            ...
            <a class="nav-link active" href="add_income.php"><i class="fas fa-plus-circle"></i> <span>Add Income</span></a>
    ...
    <div class="main-content fade-in">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2><i class="fas fa-plus-circle text-success"></i> Add Income Transaction</h2>
                ...
                <form method="POST">
                    <div class="row">
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea name="description" class="form-control" rows="2" placeholder="e.g., Customer payment, Service fee, etc." required></textarea>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label>Item Name</label>
                            <input type="text" name="item_name" class="form-control" placeholder="Product or service name">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label>Select Product (Optional)</label>
                            <select name="product_id" class="form-select">
                                <option value="">-- Not related to product --</option>
                                <?php foreach($products as $product): ?>
                                <option value="<?php echo $product['id']; ?>"><?php echo $product['product_code']; ?> - <?php echo $product['product_name']; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label>Quantity</label>
                            <input type="number" name="quantity" class="form-control" id="quantity" min="0" step="1" value="0">
                        </div>
                        <div class="col-md-4 mb-3">
                            <label>Unit Price (₱)</label>
                            <input type="number" name="unit_price" class="form-control" id="unit_price" min="0" step="0.01" value="0">
                        </div>
                        <div class="col-md-4 mb-3">
                        ...
                        <div class="col-12">
                            <button type="submit" class="btn btn-save"><i class="fas fa-save"></i> Save Income</button>
