...
$products = $pdo->query("SELECT id, product_name, product_code, quantity FROM products WHERE quantity > 0 ORDER BY product_name")->fetchAll();
$message = '';
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if($product['quantity'] < $quantity) {
        $message = '<div class="alert alert-danger">Error: Insufficient stock! Available: ' . $product['quantity'] . '</div>';
    } else {
        ...
        $trans = $pdo->prepare("INSERT INTO transactions (product_id, user_id, transaction_type, quantity, previous_quantity, new_quantity, remarks) VALUES (?, ?, 'OUT', ?, ?, ?, ?)");
        $trans->execute([$product_id, $_SESSION['user_id'], $quantity, $old_qty, $new_qty, $remarks]);
        
        $message = '<div class="alert alert-success">Stock Out successful! New quantity: ' . $new_qty . '</div>';
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Stock Out - Lombriks (Staff)</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-danger { background: linear-gradient(90deg, #f093fb, #f5576c); border: none; border-radius: 20px; padding: 10px 30px; }
        .btn-danger:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(240,147,251,0.4); }
...
<body>
    ...
    <div class="sidebar">
        ...
        <nav class="nav flex-column">
            ...
            <a class="nav-link active" href="stock_out.php"><i class="fas fa-arrow-up"></i> <span>Stock Out</span></a>
        </nav>
    ...
    <div class="main-content fade-in">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2><i class="fas fa-arrow-up text-danger"></i> Stock Out Transaction</h2>
        </div>
                ...
                <form method="POST">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="product_id" class="form-select" required>
                                ...
                                <option value="<?php echo $product['id']; ?>">
                                    <?php echo $product['product_code']; ?> - <?php echo $product['product_name']; ?> (Available: <?php echo $product['quantity']; ?>)
                                </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="col-12 mb-3">
                            <label>Remarks</label>
                            <textarea name="remarks" class="form-control" rows="3" placeholder="e.g., Sold to customer, Damaged, etc."></textarea>
                        </div>
                        <div class="col-12">
                            <button type="submit" class="btn btn-danger"><i class="fas fa-save"></i> Process Stock Out</button>
                        </div>
                    </div>
                </form>
            </div>
